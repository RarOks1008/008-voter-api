...
namespace Voter.Api.Core
{
    public class JwtManager
    {
        ...
        public JwtManager(VoterContext context, string issuer, string secretKey)
        {
            _context = context;
            _issuer = issuer;
        ...
        {
            var person = _context.Persons.Include(p => p.Role.RoleUseCases)
                .FirstOrDefault(x => x.PersonalId == username && x.Password == password);
            if (person == null)
            {
                return null;
            }
            ...
            {
                Id = person.Id,
                AllowedUseCases = person.Role.RoleUseCases.Select(x => x.UseCaseId),
